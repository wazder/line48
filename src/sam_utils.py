...
class SAMLineLogic:
    ...
    def __init__(self, 
        ...
        # Class-specific confidence thresholds - suitcase confidence lowered
        self.confidence_thresholds = {
            0: 0.85,   # person - much higher to reduce from 10 to 3
            24: 0.60,  # backpack - much lower to enable detection (0→3)
            26: 0.70,  # handbag - much higher to reduce from 8 to 1
            28: 0.45   # suitcase - much lower to enable detection (was 0.65→0.45)
        }
        ...
        for i, (detection, bbox) in enumerate(zip(detection_results, valid_boxes)):
            ...
            try:
                ...
                if len(masks) > 0:
                    ...
                    # Add label with track ID (category ID will be added later in run_first_200.py)
                    track_id = detection.get('track_id', 'N/A')
                    label = f"{detection['class']} Track:{track_id} ({detection['confidence']:.2f})"
